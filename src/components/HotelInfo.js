...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = async () => {
    ...
    const resp = await fetch(
      'https://ujixu33ov0.execute-api.eu-north-1.amazonaws.com/Production/accessibilities'
    )
  ...
  const loadServicesData = async () => {
    // Query the API Gateway
    const resp = await fetch(
      'https://ujixu33ov0.execute-api.eu-north-1.amazonaws.com/Production/services'
    )
    let jsonData = await resp.json()
